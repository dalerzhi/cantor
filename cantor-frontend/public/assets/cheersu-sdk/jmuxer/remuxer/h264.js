...
export class H264Remuxer extends BaseRemuxer {
  constructor(timescale, callback) {
    super();
    ...
    this.samples = [];
    this.h264 = new H264Parser(this, (info) => {
      callback(info);
    });
  }
  ...
  remux(frames) {
    for (let frame of frames) {
      ...
      if (units.length > 0 && this.readyToDecode) {
        ...
        // console.log(frame.keyFrame, "frame.keyFrame");
        this.samples.push({
          units: units,
          size: size,
  ...
  getPayload() {
    ...
    while (this.samples.length) {
      ...
      if (duration <= 0) {
        debug.log(
          `remuxer: invalid sample duration at DTS: ${this.nextDts} :${duration}`
        );
